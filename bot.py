...
MEISHI_ALLOWED_USERS: set[str] = set(
    ...
    for u in os.environ.get("MEISHI_ALLOWED_USERS", "kk_12345,ks19970606,kk1_12345,pinapo4444").split(",")
    if u.strip()
)
...
FAQ_ITEMS = {
    ...
    "faq_fraud": {
        "q": "Q.契約した携帯を詐欺に使うのでは？",
        "a": "詐欺には使用しません。心配なら一定期間SIMカードをお預かりしていただいて構いません。",
    },
    "faq_device": {
        "q": "Q.契約した端末はどうなりますか？",
        "a": "当グループで提携しているリセール市場に流します。",
    },
    "faq_reason": {
        "q": "Q.なぜそんな事をするのですか？",
        "a": "携帯販売会社には台数のノルマがあるため、実は一定数のサクラ取引が存在します。街角のauショップも直営店舗は「au style」だけで実はほとんどが“販売代理店”であり、当然au側から販売台数のノルマがあります。",
    },
}
